    ...
    module Interface
      ...
      class Magick
        ...
        def identify(options = {})
          ...
          raise Quirkey::Imanip::Errors::NotAnImageError, "Could not identify the image #{@image_path} as an image: #{response}" if matches.nil?
          @format = matches[1]
        ...
        def convert(to_file,options = {})
          ...
          possible_errors.each do |error|
            raise(Quirkey::Imanip::Errors::CouldNotConvertError, response + " when " + command) if response =~ error
          end
          # assert that the new file exits
          File.readable?(to_file) ? to_file : raise(Quirkey::Imanip::Errors::CouldNotConvertError)
        end
